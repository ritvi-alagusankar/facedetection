...
def main():
    ...
    # Load dataset with both detectors for comparison
    ...
    # Choose the detector that found more faces
    if len(haar_faces) >= len(hog_faces):
        print("\n[INFO] Using Haar Cascade results (found more faces)")
        faces, labels, class_counts = haar_faces, haar_labels, haar_counts
        detector_type = "haar"
    else:
        print("\n[INFO] Using HOG results (found more faces)")
    ...
    print(f"\n[INFO] Best model: {best_model_name} with accuracy {max(eigen_accuracy, fisher_accuracy):.2f}%")
